...
def sub(path, pattern, repl, flags=re.S):
    ...
    if n != 1:
        raise SystemExit(f"pattern not found exactly once in {path}: {pattern[:100]}")
    p.write_text(ns)
...
# Generic player command target: ore, enemy, or an arbitrary tunnel point.
rep(cave,
...
# No per-frame worker-list copy. Explicit user commands override autonomous AI.
sub(cave,
...
# High logistics no longer overshoots route nodes forever; unreachable routes are throttled.
sub(cave,
    r'    private float moveSpeed\(Worker w\)\{.*?\n    private Vein chooseVein\(Worker w\)\{.*?\}',
...
# Imps only steal; hostile combatants can kill.
sub(cave,
    r'    private void robChest\(Mob m\)\{.*?\n    \}\n\n    private void attackWorker\(Mob m,Worker w\)\{.*?\n    \}',
...
# Trap lethality.
sub(cave,
    r'    private void fireHazard\(CaveHazard h\)\{.*?\n    \}\n\n    private void invalidateRoutes',
...
# Particle budget and render allocation reductions for 90+ workers.
sub(cave,
    r'    private void updateFx\(float dt\)\{.*?\n    private void spawnBreak',
...
# Better collapse and water animations.
sub(cave,
...
# World tap: enemy first, ore second, otherwise point command.
sub(cave,
    r'    private boolean handleWorldTap\(float x,float y\)\{.*?\n    \}\n\n    private void buyGlobal',
...
# First-depth miner prices are deliberately tutorial-friendly.
rep(state,
...
# Every imp steals a percentage from every resource type currently present.
sub(state,
    r'    public long\[\] stealFromChest\(int depth, boolean king\) \{.*?\n    \}\n\n    public long minerBuyCost',
...
rep(cave,
    'case GNOMES->{if(state.mergeTier(selectedTier)){syncWorkers(false);toast="ЭВОЛЮЦИЯ • 10 → 1";toastTime=1.4f;}}',
    'case GNOMES->{if(state.mergeTier(selectedTier)){syncWorkers(false);toast=state.depth==1&&levelObjectiveMet()?"ЦЕЛЬ ВЫПОЛНЕНА • ПРОДВИНУТЫЙ ГНОМ":"ЭВОЛЮЦИЯ • 10 → 1";toastTime=1.6f;}}')

print("gameplay control pass applied")
